// Itinerary assistant widget. A floating, sandboxed chatbot bolted onto the
// travel map. It can ONLY read and write the itinerary documents — its tools
// are scoped to exactly that namespace, server-side.
//
// This file is the UI half: launcher + panel, conversation state, and the
// wire protocol. It POSTs the running transcript to /grubbery/itinerary/chat
...
(function () {
  ...
  var SUGGESTIONS = [
    'What is on this itinerary?',
    'Add a pin: a good lunch spot near the center',
    'Outline a zone around the old town',
  ];
  ...
  function build() {
    fab = el('button', 'da-fab da-hidden', I.spark + '<span>Ask the map… &#8984;J</span>');
    fab.title = 'Ask the map assistant (⌘J)';
    ...
    titleWrap.appendChild(el('div', 'da-title', 'Itinerary assistant'));
    titleWrap.appendChild(el('div', 'da-sub', 'reads &amp; edits your itineraries — nothing else'));
    ...
    input = el('textarea');
    input.rows = 1;
    input.placeholder = 'Add a pin, plan a day, ask anything…';
    ...
    // inline mode: if the page provides a #chat-mount container (the
    // sidebar Chat tab), live there always-open — no launcher, no float.
    var mount = document.getElementById('chat-mount');
    if (mount) {
      panel.classList.remove('da-hidden');
      panel.classList.add('da-inline');
      minBtn.style.display = 'none';
      mount.appendChild(panel);
      document.body.append(cfgBack);
      renderLog();
      loadHistory();
      return;
    }
    ...
    fetch(HISTORY)
      ...
      .then(function (arr) {
        ...
        history = arr.map(function (m) {
          return { role: m.role, content: m.content, trace: m.trace || [], parts: m.parts || [] };
        });
        renderLog();
  ...
  function send(text) {
    ...
    fetch(ENDPOINT, {
      ...
      .then(function (data) {
        ...
        history.push({ role: 'assistant', content: reply, trace: (data && data.trace) || [], parts: (data && data.parts) || [] });
        renderLog();
      })
      .catch(function (err) {
        typing.remove();
    ...
    history.forEach(function (m) {
      if (m.role === 'user') {
        ...
        if (m.parts && m.parts.length) {
          // interleave prose and tool chips in their true order
          var buf = [];
          var flush = function () {
            if (!buf.length) return;
            b.appendChild(el('div', 'da-bubble', renderMarkdown(buf.join('\n\n'))));
            buf = [];
          };
          m.parts.forEach(function (p) {
            if (p.type === 'text') { if (p.text) buf.push(p.text); }
            else { flush(); b.appendChild(traceEl([p])); }
          });
          flush();
        } else {
  ...
  function emptyState() {
    ...
    wrap.innerHTML = 'I can read and <b>edit your itinerary</b> — add pins and zones, ' +
      'plan days, answer questions about what’s on the map.';
