...
/<  selections  /lib/obelisk/selections.hoon
/<  predicate  /lib/obelisk/predicate.hoon
/<  mip  /lib/mip.hoon
=,  obelisk-types
=,  server-state
=,  utils
...
++  do-crud-txn
  |=  $:  =crud-txn:ast
          ...
          next-schemas=(map @tas @da)
      ==
  ^-  [? [(map @tas @da) db-state (list result:ast)]]
...
++  do-insert-upsert
  ...
  =/  cur-schema=schema
        %:  get-next-schema  sys.db.txn
                             ...
                             ==
  =/  effective-time=@da  (set-tmsp as-of.ins now)
  ::
  =.  tmsp.file.txn            now
  =.  ship.nxt-data.txn        our
  =.  provenance.nxt-data.txn  /
  =.  tmsp.nxt-data.txn        now
  ::
  ...
  ?~  value-table
    ...
    :+  (~(put by next-data) database.qualified-table.ins now)
...
++  do-delete
  ...
  =/  cur-schema=schema
        %:  get-next-schema  sys.db.txn
                             ...
                             ==
  =/  effective-time=@da  (set-tmsp as-of.d now)
  ...
  =/  resolved-scalars
    %:  ~(resolve-query-scalars selections [state our now eny])
          scalars.d
          named-ctes
  ...
  =.  tmsp.nxt-data.txn        now
  ::
  =.  content.db.txn     (put:data-key content.db.txn now nxt-data.txn)
  =.  view-cache.db.txn  (upd-view-caches state db.txn now ~ %delete)
  =.  state          (update-sys state now)
  ::
  :+  (~(put by next-data) database.qualified-table.d now)
...
++  do-update
  ...
  =/  cur-schema=schema
        %:  get-next-schema  sys.db.txn
                             ...
                             ==
  =/  effective-time=@da  (set-tmsp as-of.u now)
  ?.  (gth rowcount.file.txn 0)  :: don't bother if table is empty
    :+  next-data
        state
        :~  :-  %action
  ...
  ::
  =.  content.db.txn     (put:data-key content.db.txn now nxt-data.txn)
  =.  view-cache.db.txn  (upd-view-caches state db.txn now ~ %update)
  =.  state          (update-sys state now)
  ::
  :+  (~(put by next-data) database.qualified-table.u now)
      (~(put by state) name.db.txn db.txn)
      :~  :-  %action
              %-  crip
                  %+  weld  "UPDATE "
...
++  do-query
  ...
  ?~  from.q
    =/  resolved-scalars
      %:  ~(resolve-query-scalars selections [state our now eny])  scalars.q
                                                         named-ctes
                                                         *qualifier-lookup
                                                         *qualified-map-meta
                                                         ==
  ...
  ?~  joins.f  (~(select-relation selections [state our now eny]) q is-cte named-ctes)
  ::
  =/  =join-return      (~(join-all selections [state our now eny]) q named-ctes)
  ?:  is-cte
...
++  select-results
  ...
  ?~  raw  ?~  out
             :~  [%action 'SELECT']
                 [%result-set vectors]
                 [%server-time now]
                 [%schema-time created-tmsp:(~(got by db-state.join-return) %sys)]
                 [%data-time created-tmsp:(~(got by db-state.join-return) %sys)]
...
++  select-literals
  ...
  |=  [=db-state columns=(list selected-column:ast) is-cte=? =resolved-scalars]
  ^-  [join-return (list vector)]
...
++  named-queries
  ...
  =.  state             db-state.join-return
  =/  selected          (normalize-selected columns.select.cte-q)
  =/  qualifier-lookup  (mk-qualifier-lookup set-tables.join-return selected)
  =/  resolved-scalars
        %:  ~(resolve-query-scalars selections [state our now eny])  scalars.cte-q
                                                           nctes
                                                           qualifier-lookup
                                                           map-meta.join-return
                                                           ==
