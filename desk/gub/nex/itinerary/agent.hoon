...
::  set by the itinerary nexus when it mounts /agent: it may read and write
::  the itineraries documents and poke the metered provider proxy — nothing
::  else. Conversation history lives in chat.json, durable and inspectable.
...
=<  ^-  nexus:nexus
    ...
    ++  on-load
      ...
      :~  (manifest:loader 0)
          ...
          ::  the itinerary bundle (read/write itineraries). Nested here, it is
    ...
    ++  on-file
      ...
      ?+    rail  stay:m
          ...
          [~ %'main.sig']
        ;<  ~  bind:m  (rise-wait:io prod "%itinerary-agent/main: failed")
        |-
...
++  write-chat
  ...
  ~?  >>>  ?=(^ err)  %itinerary-agent-write-failed
  (pure:m ~)
...
++  system-seed
  ^-  @t
  '''
  You are the itinerary assistant, embedded in a travel-map app. Itineraries
  are JSON documents of pins (points of interest) and zones (outlined areas),
  each with a category, description, and notes. You can list and read
  itineraries, and edit ANY part of the document: pins and zones
...
::  +itin-tools: the Anthropic tool schema for the itinerary capabilities.
++  itin-tools
  ^-  json
  =/  mk-tool
  ...
  :~  ::  server-side web search: runs on Anthropic's end, no ship plumbing;
      ...
      :~  ['type' s+'web_search_20250305']
          ['name' s+'web_search']
          ['max_uses' (numb:enjs:format 3)]
      ==
      %:  mk-tool  'list_itineraries'
        'List all itineraries by id (the id is the filename without .json).'
        ~  ~
      ==
      %:  mk-tool  'read_itinerary'
        'Read one itinerary document in full by id. Returns its JSON: name, center, zoom, categories, pins, zones.'
        ~[['id' 'the itinerary id, e.g. my-trip']]  ~['id']
      ==
      %:  mk-tool  'write_entry'
        ...
        ~[['itinerary' 'the itinerary id'] ['field' '"pins" or "zones"'] ['id' 'the entry id, kebab-case'] ['entry' 'the pin or zone as a JSON object string']]
        ~['itinerary' 'field' 'id' 'entry']
      ==
      %:  mk-tool  'delete_entry'
        'Delete a pin or zone from an itinerary.'
        ~[['itinerary' 'the itinerary id'] ['field' '"pins" or "zones"'] ['id' 'the entry id to delete']]
        ~['itinerary' 'field' 'id']
      ==
      %:  mk-tool  'write_field'
      ...
      %:  mk-tool  'delete_field'
        ...
        ~[['itinerary' 'the itinerary id'] ['path' 'slash path from the document root']]
        ~['itinerary' 'path']
      ==
  ==
...
++  run-loop
  ...
  ::  record them in the trace so the UI shows the search happened.
  ...
  =.  trace
    ...
    trace
  ::  ordered parts: text and server-tool blocks in content order, so the
  ::  UI can interleave prose with the tool chips that split it.
  =.  parts
    %+  weld  parts
    ^-  (list json)
    %+  murn  content-arr
    ...
    =/  q=@t
      ...
      =/  qq  (~(get by p.input) 'query')
      ?:(?=([~ %s *] qq) p.u.qq '')
    `(pairs:enjs:format ~[['type' s+'tool'] ['tool' s+(jstr b 'name')] ['arg' s+q] ['note' s+'web']])
  =/  tool-uses=(list json)
    %+  skim  content-arr
    |=  b=json
...
++  run-tools
  ...
  =/  arg=@t
    ?.  ?=([%o *] input)  ''
    =/  eid  (~(get by p.input) 'id')
    ?:  ?=([~ %s *] eid)  p.u.eid
    =/  it  (~(get by p.input) 'itinerary')
    ?:(?=([~ %s *] it) p.u.it '')
  =/  te=json
