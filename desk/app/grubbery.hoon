...
++  on-load
  ...
  ?-    -.old
    ...
    $(old-state !>((migrate-1-to-2:migrations old)))
      %2
    $(old-state !>((migrate-2-to-3:migrations old)))
      %3
    $(old-state !>((migrate-3-to-4:migrations old)))
      %4
    =.  state  old
    =^  start-cards  state  abet:cold-start:hc
    [start-cards this]
...
++  handle-dart
  ...
  ?-    -.dart
    ...
    ?-    -.load.dart
      ...
      ::
        %tag
      ::  Set tags on current hist entry.
      ?>  ?=(%& -.u.dest-lane)
      =/  res=(each _this tang)
        (mule |.((tag-hist p.u.dest-lane case.load.dart tags.load.dart)))
      ?-  -.res
...
++  handle-lick-spin
  ...
  =.  this  (ensure-dir dir)
  ::  init /live only when absent: on a respin of an already-connected
  ::  port the socket survives in vere and no new %connect will arrive,
  ::  so a forced %.n here would go stale.
  =.  this
...
++  take-lick-soak
  ...
  ?:  =(%disconnect mark)
    (save-file [dir %live] [[/ %loob] %& !>(%.n)])
  ?:  =(%error mark)
    ::  persist runtime errors (port collision, fs perms) so callers can
    ::  observe a port that will never come up, not just scan the log.
...
++  handle-sys-poke
  ...
  ::
      %lick
    ?:  =([/ %lick-spin] p.sage)
      =.  this  (handle-lick-spin !<(path q.sage))
