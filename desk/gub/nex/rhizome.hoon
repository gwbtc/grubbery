...
=<  ^-  nexus:nexus
    ...
    ++  on-load
      ...
      %+  spin:loader  ball
      :~  (manifest:loader 0)
          [%over %& [/ %'link.json'] [[/ %json] (pairs:enjs:format ~[['name' s+'rhizome'] ['description' s+'Wiki-linked markdown notes with backlinks']])]]
    ...
    ++  on-file
      ...
      ?+    rail  stay:m
        ...
        ;<  =view:nexus  bind:m  (peek:io (cord-to-road:tarball './vault/') ~)
        =/  old-ball=ball:tarball  (ball-from-view view)
        ...
        ;<  =view:nexus  bind:m  (peek:io (cord-to-road:tarball './vault/') ~)
        =/  new-ball=ball:tarball  (ball-from-view view)
        =/  changed=(set @ta)
        ...
        ;<  vault-wave=wave:nexus  bind:m
          (keep:io /vault (cord-to-road:tarball './vault/') ~)
        ;<  vault-view=view:nexus  bind:m  (peek:io (cord-to-road:tarball './vault/') ~)
...
::
::  ball-from-view: extract ball from a peek result
::
++  ball-from-view
  |=  =view:nexus
  ^-  ball:tarball
...
++  files-from-ball-at
  ...
  =.  out
    ...
    |=  [[name=@ta =sang:tarball gain=? bang=(unit tang)] acc=(map @ta @t)]
    ?.  =(%txt name.p.sang)  acc
...
++  page-js
  ...
  ;:  weld
    ...
    "  fetch(OVER+cur+'?blot=/txt',\{method:'POST',"
    ...
    "  fetch(MAKE+name+'?blot=/txt',\{method:'PUT',"
