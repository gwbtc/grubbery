      ...
      ;script
        ;+  ;/
          ;:  weld
            ...
            "var DESCS=\{};(async function()\{try\{var r=await fetch(API+'/file/'+BASE+'/descs.json?blot=/json');if(r.ok)DESCS=JSON.parse(await r.text())}catch(e)\{}})();document.getElementById('mark').addEventListener('blur',function()\{var m=this.value.trim();if(DESCS[m])document.getElementById('desc').value=DESCS[m]});"
            ::  load latest request on init (server-side may be stale)
            "(async function()\{try\{var r=await fetch(API+'/file/'+BASE+'/request.json?blot=/json');if(!r.ok)return;var j=JSON.parse(await r.text());if(j.system)document.getElementById('system').value=j.system;if(j.prompt)document.getElementById('prompt').value=j.prompt;if(j.mark)document.getElementById('mark').value=j.mark;if(j.desc)document.getElementById('desc').value=j.desc}catch(e)\{}})();"
            ::  send: include mark and desc
            "document.getElementById('send').onclick=async function()\{var s=document.getElementById('system').value;var p=document.getElementById('prompt').value.trim();var m=document.getElementById('mark').value.trim()||'txt';var d=document.getElementById('desc').value;if(!p)\{document.getElementById('prompt').style.border='2px solid #c62828';return}document.getElementById('prompt').style.border='';document.getElementById('output').textContent='Sending...';document.getElementById('output').className='muted';await fetch(API+'/poke/'+BASE+'/main.sig?blot=/json',\{method:'POST',headers:\{'Content-Type':'application/json'},body:JSON.stringify(\{system:s,prompt:p,mark:m,desc:d})})};"
            ::  search button
            "document.getElementById('search-btn').onclick=async function()\{var p=document.getElementById('prompt').value.trim();if(!p)\{document.getElementById('prompt').style.border='2px solid #c62828';return}document.getElementById('prompt').style.border='';document.getElementById('output').textContent='Searching...';document.getElementById('output').className='muted';await fetch(API+'/poke/'+BASE+'/main.sig?blot=/json',\{method:'POST',headers:\{'Content-Type':'application/json'},body:JSON.stringify(\{action:'search',prompt:p})})};"
            ::  live-save: persist fields on blur
            "function saveFields()\{var b=\{action:'save',system:document.getElementById('system').value,prompt:document.getElementById('prompt').value,mark:document.getElementById('mark').value,desc:document.getElementById('desc').value};fetch(API+'/poke/'+BASE+'/main.sig?blot=/json',\{method:'POST',headers:\{'Content-Type':'application/json'},body:JSON.stringify(b)})}['system','prompt','mark','desc'].forEach(function(id)\{document.getElementById(id).addEventListener('input',saveFields)});"
            ...
            "function cfgLoad(f)\{cfgFile=f;document.querySelectorAll('.cfg-tab').forEach(function(b)\{b.className='cfg-tab'+(b.dataset.file===f?' active':'')});fetch(API+'/file/'+BASE+'/'+f+'?blot=/json').then(function(r)\{return r.ok?r.text():''}).then(function(t)\{try\{cfgEditor.value=JSON.stringify(JSON.parse(t),null,2)}catch(e)\{cfgEditor.value=t}})}"
            "document.getElementById('config-btn').onclick=function()\{cfgBack.classList.add('open');cfgLoad('config/claude.json')};"
            "document.getElementById('cfg-save').onclick=async function()\{try\{var j=JSON.parse(cfgEditor.value);var r=await fetch(API+'/over/'+BASE+'/'+cfgFile+'?blot=/json',\{method:'POST',headers:\{'Content-Type':'application/json'},body:JSON.stringify(j)});if(r.ok)\{cfgBack.classList.remove('open')}else\{alert('Save failed: '+r.status)}}catch(e)\{alert('Invalid JSON: '+e.message)}};"
      ...
      ;script
        ;+  ;/
          ;:  weld
            ...
            ::  brief button
            "document.getElementById('go').onclick=async function()\{var t=document.getElementById('topic').value.trim();if(!t)\{document.getElementById('topic').style.border='2px solid #c62828';return}document.getElementById('topic').style.border='';await fetch(API+'/poke/'+BASE+'/main.sig?blot=/json',\{method:'POST',headers:\{'Content-Type':'application/json'},body:JSON.stringify(\{action:'brief',prompt:t})})};"
            ::  SSE: stream briefing state
            "async function connect()\{try\{var r=await fetch(API+'/keep/'+BASE+'/briefing.json?blot=/txt',\{headers:\{Accept:'text/event-stream'}});var R=r.body.getReader();var d=new TextDecoder();var buf='';while(true)\{var c=await R.read();if(c.done)break;buf+=d.decode(c.value,\{stream:true});var ps=buf.split('\\n\\n');buf=ps.pop();for(var i=0;i<ps.length;i++)\{if(!ps[i].trim())continue;var data='',ls=ps[i].split('\\n');for(var j=0;j<ls.length;j++)\{if(ls[j].indexOf('data: ')===0)data+=ls[j].slice(6)}if(!data)continue;try\{var j=JSON.parse(data);"
